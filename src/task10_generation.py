...
def generate_with_citation(query: str, top_k: int = TOP_K) -> dict:
    ...
    if not answer:
        if reordered:
            sources = [chunk.get("metadata", {}).get("source", "unknown") for chunk in reordered[: min(len(reordered), 3)]]
            answer = (
                f"Dựa trên các nguồn liên quan, tôi tìm thấy thông tin phù hợp từ: {', '.join(sources)}."
            )
        else:
