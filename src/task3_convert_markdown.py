...
def convert_legal_docs():
    ...
    for filepath in legal_dir.iterdir():
        if filepath.suffix.lower() in (".pdf", ".docx", ".doc"):
            ...
            try:
                content = md.convert(str(filepath)).text_content if md else ""
            except Exception as error:
                print(f"  ⚠ MarkItDown không đọc được {filepath.name}, dùng DOCX fallback: {error}")
                content = ""
            if not content and filepath.suffix.lower() == ".docx":
                with ZipFile(filepath) as archive:
                    xml = archive.read("word/document.xml").decode("utf-8")
                content = re.sub(r"<[^>]+>", " ", xml).replace("  ", " ").strip()
            elif not content:
                content = "Không thể convert PDF: cài đặt markitdown[pdf] để xử lý file này."
